# S-DEC-by-qt项目介绍
这是一个关于实现S-DES算法的加密和解密程序的项目。S-DES是一种对8位数据进行加密和解密的算法，使用10位密钥进行操作。

项目要求分为几个关卡：

1. 第1关：基本测试。编写并调试程序，实现S-DES算法的加密和解密功能，并提供GUI界面以支持用户交互。输入为8位数据和10位密钥，输出为8位密文。

2. 第2关：交叉测试。确保所有人使用相同的算法流程和转换单元（如P-Box、S-Box）编写程序，以确保算法和程序在不同系统或平台上都能正常运行。通过A组和B组的交叉测试，验证加密程序得到相同的密文，解密程序能还原出相同的明文。

3. 第3关：扩展功能。允许输入ASCII编码的字符串作为数据输入，输出也可以是对应的ASCII字符串（可能是乱码）。

4. 第4关：暴力破解。假设你已经获得了使用相同密钥的一个或多个明文和密文对，尝试使用暴力破解的方法找到正确的密钥。你可以考虑使用多线程来提高破解效率，并记录时间戳以展示破解所需的时间。

5. 第5关：封闭测试。根据第4关的结果，进一步分析对于随机选择的一个明文密文对，是否存在不止一个密钥可以得到相同的密文。同时，对于给定的明文分组Pn，是否存在不同的密钥Ki和Kj可以得到相同的密文Cn。

在编写代码时，遵循了代码规范，使用了有意义的变量命名，进行了适当的代码注释，并使用函数式编程来实现模块化和代码复用。
